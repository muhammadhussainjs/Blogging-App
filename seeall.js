...
function handleBrowserBack() {
    getDataFromuser(); // Fetch and display the user's data again
}

// Add event listener for "Back to All" button
back.addEventListener('click', handleBackToAll);
...
function renderPost() {
    ...
    array.forEach((item) => {
        ...
        ` <div class="bg-[#ffffff] border-2 border-inherit mt-12 rounded-xl pb-8 mb-12 shadow-xl pl-3 sm:pl-5">
            ...
            <div>   
                <p class="text-[#4d4a4a] text-[14px] font-light mt-2 whitespace-normal break-words">${item.area}</p>
            </div>
        </div>`;
    });
}
